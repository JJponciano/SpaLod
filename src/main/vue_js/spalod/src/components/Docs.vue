...
<p>Storing geospatial data for widespread use and sharing is a challenging research problem, primarily because of the rapid growth of data, at least by 20% every year (Lee and Kang, 2015). In various studies, authors present an ontology-enabled framework to find geographic services aiming to solve geospatial problems. However, their ontologies do not provide a common vocabulary to describe geographic features and attributes, limiting their utility.</p>

<p>While these research projects do not group heterogeneous spatial data in a knowledge base at an advanced level and using a common vocabulary, projects such as (Jung et al., 2013; Budak Arpinar et al., 2006; Dsouza et al., 2021) demonstrate that intelligent data provisioning using Semantic Web methods appears to be an effective solution for simplified linking of official and unofficial data for geodata users.</p>

<p>Moreover, projects using ontology-based frameworks such as (Dsouza et al., 2021; Sun et al., 2019; Karalis et al., 2019) highlight the growing interest in structuring geospatial data in the form of knowledge graphs. Such graphs are stored in triplestores (such as Apache Marmotta 1, Apache Jena 2, Eclipse RDF4J 3, Strabon 4, Oracle Spatial and Graph 5, GraphDB 6, Stardog 7, and Virtuoso Universal Server 8) allowing fast read and write access by SPARQL.</p>

<p>In the domain of geospatial, the GeoSPARQL (Battle and Kolas, 2011) approach has greatly improved access to geospatial data from a triplestore, allowing to retrieve and update geospatial data with simplified queries. However, GeoSPARQL requires that the stored data be structured according to a particular form in order to be usable, but most of the triplestores from the LOD have not added the structure proposed by GeoSPARQL. This constraint limits the use of GeoSPARQL for the LOD.</p>

<p>The other main issue is the heterogeneity of the geospatial data. Usually these data are stored in specific formats (such as GeoJSON (Butler et al., 2016), Shapefile 9, OpenStreetMap (Ramm et al., 2014)) having their own structure and being in different languages. This variation in vocabulary poses a major challenge for the integration of data from different sources.</p>

<p>We can deduce from this study that (i) the use of knowledge base (in the form of ontology and triplestore) is the most flexible and promising way to store large amounts of geospatial data, (ii) that there is currently no known platform to efficiently group heterogeneous spatial data in a knowledge base with a common vocabulary for all integrated datasets, (iii) that the major challenge for the integration of geospatial data is the diversity of structures and vocabularies used to organize and express this information.</p>
...
In section 3.1, we highlight solution for the integration of heterogeneous geospatial data into a Universal Spatial Knowledge Base (USKB). In Section
...
OpenStreetMap, RDF (Pan, 2009). The work (Patroumpas et al., 2014) proposed such conversion without considering other ontologies. Thus, they are limited to the vocabulary used in the data file for creating
...
et al., 2014), Owlgres (Stocker and Smith, 2008), Pellet (Sirin et al., 2007), DeLorea (Bobillo et al., 2012)).</p>
...
knowledge in one place and thus, facilitating the retrieval of geographic data. Access to the data knowledge can then be done via a web interface.
...
fully exploiting the power of linked open data to provide a varied and structured vocabulary in the English language. Recently DCAT- AP (Kirstein et al.,
...
Since the vocabulary comes from various languages, we propose to use a Neural Machine Translation approach to automatically translate each term
...
knowledge base vocabulary using an ontology alignment approach such as (Zhang et al., 2014). Following this alignment, the aligned terms between the vocabulary used in the RDF files and the vocabulary of
...
schema:faxNumber, schema:name thanks to the English translation done before. Finally, the individual will be linked to its geometry with the property geo:hasGeometry. Its geometry will be of type
...
into this ontology using structured mapping. The consistency of the data integration is ensured by a reasoning applied on the constraints defined for each
...
experts. The exploitation of expert knowledge modelled in an ontology can be considered in order to automate this evaluation, e.g. by generating correspondence concepts to semantically map the different data
...
to wordnet. In Vetulani, Z., Uszkoreit, H., and Kubis, M., editors, Human Language Technology. Challenges for Computer Science and Linguistics, pages
...
pages 107–115.</li>
      <li>
Quarati, A., De Martino, M., and Rosim, S. (2021). Geospatial open data usage and metadata quality. ISPRS International Journal of Geo-Information, 10(1):30.</li>
      <li>
...
Proceedings of the Twenty-Ninth International Conference on International Joint Conferences on Artificial Intelligence, pages 4039–4045.</li>
    </ul>
  </div>
    ...
    <p>As depicted in the figure above, the first step for integrating and enriching the knowledge base is to retrieve an RDF representation of the data to be integrated, a process also known as uplift. When integrating data from Linked Open Data, a query is made to retrieve the desired data as an RDF triple.</p>

    <p>The retrieved RDF files can then be integrated via the enrichment web page provided. The RDF files are parsed to give the user a match between the vocabulary used in the RDF files and the vocabulary of the knowledge base. The user can validate and complete the vocabulary match to enable the integration of the RDF files to enrich the knowledge base.</p>

    <p>Integration of data is achieved using Sparql queries to update the Knowledge Base. Once the integration is complete, the data can be evaluated either automatically or manually. In the case of automatic evaluation, the data is analyzed to define and assign a ranking. This ranking is added via a Sparql query. With manual evaluation, the user can retrieve their chosen data via Sparql queries and define a ranking in the provided interface.</p>

    <p>The ranking defined by the user is then entered into the knowledge base via a Sparql query. Finally, the user can access the content of the knowledge base via a Sparql endpoint, available in a web interface.</p>
