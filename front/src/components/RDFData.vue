  ...
  <div class="rdf-data" :class="{ dark: isDarkMode }" v-if="rdfView">
    <div class="header">
      <div class="title">
        ...
        <h2>|</h2>
        <h2 :class="{ selected: !rdfView }" @click="rdfView = false">
          Query Result
        </h2>
      </div>
      ...
      <div v-if="rdfData && rdfData.length > 0" class="download">
        ...
        <button class="download-button" @click="downloadOwl">
          Download Owl
        </button>
      </div>
    </div>
      ...
      <div class="metadata-container" v-if="showMetadata">
        ...
        <div class="metadata-Catalog">
          ...
          <select
            ...
            v-bind:disabled="isCatalogDisabled"
          >
    ...
    <div class="select-all" v-if="rdfData && rdfData.length > 0">
      <input
        ...
        @change="selectAll(areAllSelected)"
      />
      <h3>Select all</h3>
    </div>
    <p v-for="(triplet, index) in rdfData" :key="triplet.id">
      ...
      <input
        type="text"
        v-model="triplet.object"
        class="predicate"
        :class="{ unknown: unkownPredicates.includes(triplet.object) }"
        @input="filterResults(triplet.object, index)"
        @focus="activeInput = index"
        :title="
          unkownPredicates.includes(triplet.object)
    ...
    <button
      :id="'btn' + index"
      class="add-button"
      @click="addTriplet(triplet, index)"
    >
      Add
    </button>
    <br
      v-if="
        rdfData[index + 1] &&
        rdfData[index + 1].subject !== rdfData[index].subject
      "
    />
    <br
      v-if="
        ...
        <tr v-for="(result, index) in queryResult" :key="index">
          ...
          >
            <template v-if="key === 'JSON'">
              <button @click="downloadJson(result.JSON, result.Feature)">
                DOWNLOAD JSON
              </button>
            </template>
...
export default {
      ...
      queryables: [
        {
          q: "identifier",
          required: true,
          d: "The unique identifier of the dataset",
          v: false,
  ...
  mounted() {
    ...
    const fetchData = async () => {
      ...
      const data = {
        query:
          "SELECT ?catalog ?title ?description ?publisher ?dataset where {?catalog <http://www.w3.org/ns/dcat#dataset> ?dataset . ?catalog <http://purl.org/dc/terms/title> ?title . ?collection <http://purl.org/dc/terms/description> ?description . ?collection <http://purl.org/dc/terms/publisher> ?publisher . ?collection <http://www.w3.org/ns/dcat#dataset> ?dataset .}",
      ...
      $ajax({
        ...
        type: "POST",
        url: import.meta.env.VITE_APP_API_BASE_URL + "/api/sparql-query/",
      ...
      const datadcat = {
        query:
          "SELECT  ?dataset where { ?dataset rdf:type <http://www.w3.org/ns/dcat#Dataset>}",
    ...
    const checkPredicates = {
      query:
        "SELECT ?type WHERE { <" +
        this.queryables.find((queryable) => queryable.q === "identifier").p +
    ...
    if (queryString.includes("/items/")) {
      ...
      if (queryString && queryString.length > 0) {
        ...
        const data = {
          query:
            "SELECT ?title WHERE { ?catalog <http://www.w3.org/ns/dcat#dataset> <spalod:" +
            queryString +
            "> . ?catalog <http://purl.org/dc/terms/title> ?title . }",
          triplestore:
            import.meta.env.VITE_APP_GRAPH_DB + "/repositories/Spalod",
        };
        $ajax({
  ...
  computed: {
    keys() {
      if (this.queryResult.length > 0) return Object.keys(this.queryResult[0]);
    },
  },
  methods: {
    getUsername() {
      const data = {
        query:
          "SELECT ?s ?p ?o WHERE {?s ?p ?o. FILTER (?s = <spalod:" +
          this.uid +
          ">)}",
        triplestore: import.meta.env.VITE_APP_GRAPH_DB + "/repositories/Spalod",
      };
    ...
    loadPredicates() {
      const data = {
        query:
          'SELECT ?property ?propertyType WHERE{{?property a owl:ObjectProperty . BIND("Object Property" AS ?propertyType)} UNION {?property a owl:DatatypeProperty . BIND("Data Property" AS ?propertyType)}} ORDER BY ?property',
        triplestore: import.meta.env.VITE_APP_GRAPH_DB + "/repositories/Spalod",
      };
      $ajax({
        headers: {
        ...
        dataType: "json",
        success: (data) => {
          this.predicateOptions = data.results.bindings.map(
            (binding) => binding.property.value
          );
      ...
      fileReader.onload = () => {
        ...
        geoJson.features.forEach((feature) => {
          ...
          if (coordinates.length > 0) {
            ...
            feature.properties["X Koordina"] &&
            feature.properties["Y Koordina"]
          ) {
            const predicate = "coordinates";
            const object =
              feature.properties["X Koordina"] +
    ...
    refreshMap() {
      ...
      const blob = new Blob([JSON.stringify(geojones)], {
        type: "application/json",
      });
      const updatefile = new File([blob], "geodata.json", {
        type: "application/json",
      });
        ...
        this.updateTripleData(tripleData, "add", () => {
          ...
          const data = {
            query:
              ...
              predicate +
              "> ?o . FILTER(?s = <" +
              String(triplet.subject).replace(/ /g, "_") +
              ">)}",
            triplestore:
              import.meta.env.VITE_APP_GRAPH_DB + "/repositories/Spalod",
          };
    ...
    filterResults(predicate, index) {
      ...
      this.filteredResults = this.predicateOptions.filter((result) =>
        result.toLowerCase().includes(predicate.toLowerCase())
      );
    ...
    getGeoJSON() {
      ...
      const subjects = [
        ...new Set(this.rdfData.map((triplet) => triplet.subject)),
      ];
    ...
    downloadOwl() {
      ...
      const file = new File([blob], "geodata.json", {
        type: "application/json",
      });
      if (file) {
    ...
    validateForm() {
      const requiredQueryables = this.queryables.filter(
        (queryable) => queryable.required
      );
      const invalidQueryables = requiredQueryables.filter(
        (queryable) => queryable.v === false
      );
    ...
    validateMetadata(data) {
      ...
      if (
        this.metadata[queryable.q] !== "" &&
        this.metadata[queryable.q] !== undefined &&
        this.selectedOption !== ""
      ) {
        if (queryable.q === "identifier") {
          var data = {
            subject:
              "spalod:" +
              this.options.find((option) => option.name === this.selectedOption)
                .id,
            predicate: this.queryables.find(
              (queryable) => queryable.q === "title"
            ).p,
            object: String(
              this.options.find((option) => option.name === this.selectedOption)
                .name
          ...
          data = {
            subject:
              ...
              this.options.find((option) => option.name === this.selectedOption)
                .id,
            predicate: this.queryables.find(
              (queryable) => queryable.q === "description"
            ).p,
            object: this.options.find(
          ...
          data = {
            subject:
              ...
              this.options.find((option) => option.name === this.selectedOption)
                .id,
            predicate: this.queryables.find(
              (queryable) => queryable.q === "publisher"
            ).p,
            object: this.options
              .find((option) => option.name === this.selectedOption)
              .publisher.startsWith("spalod:")
              ? this.options.find(
                  (option) => option.name === this.selectedOption
                ).publisher
              : "spalod:" +
        ...
        if (queryable.q === "publisher") {
          ...
          data = {
            query:
              "SELECT ?o WHERE{?s <http://xlmns.com/foaf/0.1/name> ?o . FILTER(?s = <" +
              "spalod:" +
              String(this.metadata[queryable.q]).replace(/ /g, "_") +
              ">)}",
            triplestore:
              import.meta.env.VITE_APP_GRAPH_DB + "/repositories/Spalod",
          };
          ...
          String(this.metadata[queryable.q])
            .split(",")
            .forEach((keyword) => {
              // Add the new triplets
              var tripleData = {
          ...
          var tripleData = {
             ...
             */
            object: encodeURIComponent(this.metadata[queryable.q]).replace(
              /%20/g,
              "_"
            ),
          };
      ...
      if (uri.startsWith("https://") || uri.startsWith("http://")) {
        if (uri.startsWith("http://www.opengis.net/")) {
          ...
          window.location.href =
            "/collections/" + collectionId + "/items/" + uri.split("#")[1];
        } else if (head === "HTML") {
