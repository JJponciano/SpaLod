  ...
  <div class="rdf-data" :class="{ dark: isDarkMode }" v-if="rdfView">
      ...
      <div class="metadata-container" v-if="showMetadata">
        ...
        <div v-for="(queryable, index) in queryables " :key="index" class="metadata-element">
          ...
          <div class="metadata-input">
            <input type="text" v-model="metadata[queryable.q]" class="metadata-textbox" :placeholder="queryable.d"
              @focus="$event.target.select()" @input="queryable.v = false" spellcheck="false">
            <button :id="queryable.q" class="validate" @click="validateMetadata(queryable.q)"
              :class="{ added: queryable.v }" :disabled="queryable.q !== 'identifier' && !queryables[0].v"
              v-show="queryable.q === 'identifier' || queryables[0].v">{{ queryable.v ? 'Validated' : 'Validate'
              }}</button>
          </div>
        </div>
      </div>
    ...
    <p v-for="(triplet, index) in rdfData" :key="triplet.id">
      ...
      <input type="text" v-model="triplet.object" class="predicate"
        :class="{ unknown: unkownPredicates.includes(triplet.object) }" @input="filterResults(triplet.object, index)"
        @focus="activeInput = index" :title="unkownPredicates.includes(triplet.object)
    ...
    <table>
      ...
      <tbody>
        <tr v-for="(result, index) in queryResult" :key="index">
          <td v-for="key in keys" @click="uriClick(result[key], key)"
